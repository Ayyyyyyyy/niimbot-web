...
export async function discoverOllamaModels(host = DEFAULT_OLLAMA_HOST) {
  ...
  if (isSecure) {
    try {
      ...
      throw new Error(
        `Cannot connect to Ollama via proxy. Make sure Ollama is running on the server and the Vite proxy is configured correctly.`,
      )
    }
  }
 ...
 * Returns { response, log } where log contains request/response details
 */
async function generateWithGemini(prompt, apiKey, model, signal, labelSize) {
  const startTime = Date.now()
  const timestamp = new Date().toLocaleTimeString()
...
function parseResponse(response) {
  ...
  try {
    ...
    normalized.objects = normalized.objects.map((obj) => {
      ...
      if (obj.font_size !== undefined && obj.fontSize === undefined)
        normalized_obj.fontSize = obj.font_size
      if (obj.font_weight !== undefined && obj.fontWeight === undefined)
        normalized_obj.fontWeight = obj.font_weight
      if (obj.text_align !== undefined && obj.textAlign === undefined)
        normalized_obj.textAlign = obj.text_align
