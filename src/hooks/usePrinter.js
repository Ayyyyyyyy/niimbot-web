import { ImageEncoder, NiimbotBluetoothClient, NiimbotSerialClient } from '@mmote/niimbluelib'
...
export function usePrinter() {
  ...
  const connect = useCallback(
    async (transport = 'ble') => {
      ...
      try {
        ...
        const client =
          transport === 'ble' ? new NiimbotBluetoothClient() : new NiimbotSerialClient()
  ...
  const reconnect = useCallback(async () => {
    ...
    try {
      ...
      if (err.message?.includes('GATT') || err.message?.includes('connect')) {
        setError(
          'Reconnect failed. The printer may be connected elsewhere. Try "Bluetooth" to pair again.',
        )
      } else {
  ...
  const print = useCallback(
    async (canvas, options = {}) => {
      const { direction = 'left', quantity = 1, printerType = null } = options
