import { Bot, Loader2, Send, Sparkles, StopCircle, X } from 'lucide-react'
import { useCallback, useEffect, useRef, useState } from 'react'
import { generateLabelDesign, getLLMSettings } from '../services/llmService'
...
export function AIAssistant({ isOpen, onClose, onApplyLabel, labelWidth = 50, labelHeight = 15 }) {
  ...
  const handleGenerate = useCallback(
    async (promptOverride = null) => {
      ...
      try {
        const result = await generateLabelDesign(textToUse, abortControllerRef.current.signal, {
          width: labelWidth,
          height: labelHeight,
        })
        ...
        setHistory((prev) => [
          ...prev,
          { role: 'assistant', content: JSON.stringify(labelData, null, 2) },
        ])
  ...
  const handleSurprise = useCallback(() => {
    handleGenerate(
      'Surprise me! Generate a creative, random label design using one of your themes. Be unique.',
    )
  }, [handleGenerate])

  // Check if AI is configured
  const isConfigured =
    settings.provider === 'ollama' ? !!settings.ollamaModel : !!settings.geminiApiKey
          ...
          {history.length === 0 && (
            <div className='flex flex-col items-center justify-center h-full text-center p-8 opacity-60'>
              ...
              <p className='text-gray-400 max-w-md mx-auto mb-8 text-sm leading-relaxed'>
                "Design a 10mg Retatrutide vial label"
                <br />
                "Make a hazard warning label for 100mg Caffeine"
                <br />
              ...
              <div className='bg-gray-800 rounded-2xl p-4 flex items-center gap-4 rounded-bl-none'>
                ...
                <div className='space-y-1'>
                  ...
                  <div className='text-xs text-gray-500'>
                    Analyzing dimensions & applying themes
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className='px-6 py-4 bg-green-500/5 border-t border-green-500/10 flex items-center justify-between'>
            <div className='flex items-center gap-3'>
              ...
              <div className='text-sm text-green-300'>
                <span className='font-bold'>Design Ready:</span> {generatedLabel.labelSize?.width}×
                {generatedLabel.labelSize?.height}mm
              </div>
            </div>
            <button
              type='button'
        ...
        <div className='p-6 pt-2 bg-gradient-to-t from-panel-bg to-transparent'>
          ...
          >
            ...
            <input
              ...
              onKeyDown={(e) =>
                e.key === 'Enter' && !e.shiftKey && !isGenerating && handleGenerate()
              }
