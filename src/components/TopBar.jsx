      ...
      <div className='flex items-center justify-between gap-4 flex-wrap'>
        ...
        <div className='flex items-center gap-2'>
          {!isConnected ? (
            ...
            <button onClick={onDisconnect} className='btn-secondary flex items-center gap-2'>
              <Power size={16} />
              Disconnect
            </button>
          )}
          ...
          <div className='flex items-center gap-2 px-3 py-2 bg-canvas-bg rounded-lg'>
            ...
            {printerModel && <span className='text-xs text-gray-500'>({printerModel})</span>}
            {batteryLevel !== null && (
        ...
        <div className='flex items-center gap-2'>
          ...
          <div className='flex items-center gap-3'>
            ...
            <button
              ...
              className={`relative w-11 h-6 rounded-full transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-highlight bg-gray-700 ${
                autoSaveBeforePrint ? 'bg-purple-600' : 'bg-gray-600'
              }`}
              title='Toggle Auto-save'
            >
              <span
                className={`absolute left-0.5 top-0.5 inline-block w-5 h-5 transform bg-white rounded-full shadow transition-transform duration-200 ease-in-out ${
                  autoSaveBeforePrint ? 'translate-x-5' : 'translate-x-0'
                }`}
              />
            </button>
          </div>
        ...
        <div className='mt-3 bg-accent/30 rounded-lg p-2 flex items-center gap-4 text-sm'>
          ...
          <span className='text-gray-300'>
            ...
            <span className='text-white font-medium'>
              {rfidInfo.usedPaper} / {rfidInfo.allPaper}
            </span>{' '}
            labels used
          </span>
          {rfidInfo.barCode && <span className='text-gray-400'>Code: {rfidInfo.barCode}</span>}
        </div>
      )}
